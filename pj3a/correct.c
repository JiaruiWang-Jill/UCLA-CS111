...
void analyze_direct_entry(__u32 parent_inode_index, __u32 block_index) {
  ...
  for (__u32 logical_byte_offset = 0; logical_byte_offset < block_size;
    ...
    if (dir_entry.inode != 0) {
      ...
      printf("DIRENT,%d,%d,%d,%d,%d,'%s'\n",
             parent_inode_index,   // parent inode number
...
void analyze_indirect_entry(__u32 inode_index, __u32 block_pointer, short depth,
                            short current_depth) {
  __u32 *indir_block_pointers = malloc(block_size);
  __u32 num_pointers = block_size / sizeof(__u32);
  pread(fs_fd, indir_block_pointers, block_size, block_offset(block_pointer));
  for (__u32 j = 0; j < num_pointers; j++) {
    if (indir_block_pointers[j] != 0) {
      printf("INDIRECT,%d,%d,%d,%d,%d\n",
             inode_index,    // inode number
             2,              // level of indirection
             256 + 12 + j,   // logical block offset
             block_pointer,  // block number of indirect block being scanned
             indir_block_pointers[j]  // block number of reference block
      );

      if (current_depth < depth)
        analyze_indirect_entry(inode_index, indir_block_pointers[j], depth,
                               current_depth + 1);
    }
  }
  free(indir_block_pointers);
}
...
void analyze_inode(__u32 inode_index, unsigned long inode_offset) {
  ...
  if (inode.i_block[12] != 0) {
    __u32 *block_pointers;
    _m(block_pointers = malloc(block_size), "block pointers");
    pread(fs_fd, block_pointers, block_size, block_offset(inode.i_block[12]));
    for (__u32 j = 0; j < block_size / sizeof(__u32); j++)
      if (block_pointers[j] != 0) {
        if (filetype == 'd')
          analyze_direct_entry(inode_index, block_pointers[j]);
        printf(
            "INDIRECT,%d,%d,%d,%d,%d\n",
            inode_index,        // inode number
            ...
            block_pointers[j]   // block number of reference block
        );
      }
    free(block_pointers);
  }
  ...
  /* analyze_indirect_entry(inode_index, inode.i_block[13], 2, 1); */
  if (inode.i_block[13] != 0) {
    __u32 *indir_block_pointers = malloc(block_size);
    __u32 num_pointers = block_size / sizeof(__u32);
    unsigned long indir2_offset = block_offset(inode.i_block[13]);
    pread(fs_fd, indir_block_pointers, block_size, indir2_offset);
    for (__u32 j = 0; j < num_pointers; j++) {
      if (indir_block_pointers[j] != 0) {
        printf(
            "INDIRECT,%d,%d,%d,%d,%d\n",
        ...
        __u32 *block_pointers = malloc(block_size);
        unsigned long indir_offset = block_offset(indir_block_pointers[j]);
        pread(fs_fd, block_pointers, block_size, indir_offset);

        for (__u32 k = 0; k < num_pointers; k++) {
          if (block_pointers[k] != 0) {
            if (filetype == 'd')
              analyze_direct_entry(inode_index, block_pointers[k]);
            printf("INDIRECT,%d,%d,%d,%d,%d\n",
                   inode_index,              // inode number
                   1,                        // level of indirection
                   256 + 12 + k,             // logical block offset
                   indir_block_pointers[j],  // block number of indirect block
                   // being scanned
                   block_pointers[k]  // block number of reference block
            );
          }
        }
        free(block_pointers);
  ...
  if (inode.i_block[14] != 0) {
    __u32 *indir2_block_pointers = malloc(block_size);
    __u32 num_pointers = block_size / sizeof(__u32);

    unsigned long indir3_offset = block_offset(inode.i_block[14]);
    pread(fs_fd, indir2_block_pointers, block_size, indir3_offset);

    for (__u32 j = 0; j < num_pointers; j++) {
      if (indir2_block_pointers[j] != 0) {
        printf(
            "INDIRECT,%d,%d,%d,%d,%d\n",
            inode_index,           // inode number
            ...
            indir2_block_pointers[j]  // block number of reference block
        );

        __u32 *indir_block_pointers = malloc(block_size);
        unsigned long indir2_offset = block_offset(indir2_block_pointers[j]);
        pread(fs_fd, indir_block_pointers, block_size, indir2_offset);

        for (__u32 k = 0; k < num_pointers; k++) {
          if (indir_block_pointers[k] != 0) {
            printf("INDIRECT,%d,%d,%d,%d,%d\n",
                   inode_index,               // inode number
                   2,                         // level of indirection
                   65536 + 256 + 12 + k,      // logical block offset
                   indir2_block_pointers[j],  // block number of indirect block
                   // being scanned
                   indir_block_pointers[k]  // block number of reference block
            );
            __u32 *block_pointers = malloc(block_size);
            unsigned long indir_offset = block_offset(indir_block_pointers[k]);
            pread(fs_fd, block_pointers, block_size, indir_offset);

            for (__u32 l = 0; l < num_pointers; l++) {
              if (block_pointers[l] != 0) {
                if (filetype == 'd') {
                  analyze_direct_entry(inode_index, block_pointers[l]);
                }
                printf(
                    "INDIRECT,%d,%d,%d,%d,%d\n",
                    inode_index,              // inode number
                    ...
                    indir_block_pointers[k],  // block number of indirect block
                    // being scanned
                    block_pointers[l]  // block number of reference block
                );
              }
            }
            free(block_pointers);
